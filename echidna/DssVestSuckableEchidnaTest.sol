    ...
    function vest(uint256 id) public {
        ...
        try sVest.vest(id) {
            if (block.timestamp < award.clf) {
                ...
                assert(stbl.totalSupply() == add(supplyBefore, unpaidAmt));
                assert(stbl.balanceOf(award.usr) == add(usrBalanceBefore, unpaidAmt));
            }
        } catch Error(string memory errmsg) {
            bytes32 sLocked = hevm.load(address(sVest), bytes32(uint256(4)));      // Load memory slot 0x4
            assert(
    ...
    function vest_amt(uint256 id, uint256 maxAmt) public {
        ...
        try sVest.vest(id, maxAmt) {
            if (block.timestamp < award.clf) {
                ...
                assert(stbl.totalSupply() == add(supplyBefore, amt));
                assert(stbl.balanceOf(award.usr) == add(usrBalanceBefore, amt));
            }
        } catch Error(string memory errmsg) {
            bytes32 sLocked = hevm.load(address(sVest), bytes32(uint256(4)));      // Load memory slot 0x4
            assert(
