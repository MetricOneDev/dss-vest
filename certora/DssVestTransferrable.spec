...
// certoraRun src/DssVest.sol:DssVestTransferrable certora/Stablecoin.sol --link DssVestTransferrable:gem=StableCoin --verify DssVestTransferrable:certora/DssVestTransferrable.spec --rule_sanity
...
methods {
    ...
    stbl.allowance(address, address) returns (uint256) envfree
}
...
rule vest(uint256 _id) {
    ...
    uint256 czarBalanceBefore = stbl.balanceOf(_czar);
    uint256 usrBalanceBefore = stbl.balanceOf(usr);
    ...
    uint256 czarBalanceAfter = stbl.balanceOf(_czar);
    uint256 usrBalanceAfter = stbl.balanceOf(usr);
...
rule vest_revert(uint256 _id) {
    ...
    uint256 locked = lockedGhost();
    address _czar = czar();
    uint256 czarBalance = stbl.balanceOf(_czar);
    uint256 usrBalance = stbl.balanceOf(usr);
    uint256 allowed = stbl.allowance(_czar, currentContract);
